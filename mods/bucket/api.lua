...
local function on_bucket_place(itemstack, placer, pointed_thing)
	...
	local item_def = minetest.registered_items[itemstack:get_name()]
	local bucket_def = item_def._bucket

	if check_protection(lpos, placer
			and placer:get_player_name()
			or "", "place "..bucket_def.liquid) then
		return
	end

	minetest.set_node(lpos, {name = bucket_def.liquid})
	return ItemStack(bucket_def.bucket_empty)
end
